...
function updatePluginHeaderVersion( buildDir, suffix ) {
	...
	const pluginFileName = 'ahentic.php'
...
function updateReadmeStableTag( buildDir ) {
	const pluginFilePath = path.join( buildDir, 'ahentic.php' )
	...
	const packagedVersionMatch = pluginContent.match( /^\s*\*\s*Version:\s*([^\r\n]+)/m )
	if ( ! packagedVersionMatch ) {
		return
	}

	const version = packagedVersionMatch[ 1 ].trim()
	const readmeContent = fs.readFileSync( readmePath, 'utf8' )

	if ( ! /^Stable tag:/m.test( readmeContent ) ) {
	...
	if ( updated === readmeContent ) {
		console.log( `✅ Stable tag in readme.txt already set to ${ version }` )
		return
	}
