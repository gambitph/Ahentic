		...
		public static function register() {
			...
			$defs = array(
				...
				array(
					...
					'description' => __( 'Returns the block tree from the open block editor (ref, name, preview, content_attr, attribute_keys, innerBlocks), capped for size. Full attribute values are omitted by default — use "preview" to match user phrases and "content_attr" as the attribute key to patch via update-block-attributes. Blocks with image-looking media also include compact attributes (real keys such as url/alt/id, mediaUrl/mediaAlt/mediaId, imageUrl, or other image URL fields) so you can call ahentic/describe-image and update alt without include_attributes. Pass include_attributes:true (ideally with a narrow root_ref) for other full attribute values, e.g. before a non-text style edit. Pass refs/ref to return ONLY those blocks (attributes included by default) instead of the whole document. Use returned ref values (b1, b2, …) in later browser tools — never invent clientId hashes. Runs in the browser.', 'ahentic' ),
					'meta'        => $readonly_meta,
					'input'       => array(
						'type'       => 'object',
						'properties' => array(
