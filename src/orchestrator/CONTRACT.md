...
**Think/debug module:** `Ahentic_Think_Debug` owns AHENTIC_DEBUG recovery and post-think disposition. Primary interface: `run_think()`, `apply_live_progress()`, `finalize_result_text()`, `should_finish_without_tools()`, `publish_thought_process()`, `queue_missing_ability()`, `capability_requests_for_finish()`. Internal retries (attempt ≥2) must use a slim `for_llm( …, slim_debug_retry )` prompt — not a second full backpack. Slim recovery must not decide capability gaps (`missing_ability`); after a usable think that signals `missing_ability`, challenge the claim (rewrite to `use_tools` when the ability already appears in `tools_planned` with args, otherwise one full-catalog reconsider with `full_ability_catalog` so pack-gated tools are visible) before finishing or queuing a capability request. Never inject an empty `tools_planned` call for an available ability — that yields a useless HITL card. Queue capability requests only for concrete unknown ability slugs — not vague / `ahentic/new-ability` placeholders. Attach queued requests to the idle assistant message only when the finish still signals a missing ability (`capability_requests_for_finish`); successful `reply` finishes after recovery must not surface a request card. The Orchestrator must call these — do not reimplement debug retry or missing-ability policy at call sites. Single LLM phases remain `Ahentic_Orchestrator::run_llm_phase()` (used by Think/Debug and plan-retry).
