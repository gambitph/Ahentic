...
**Prompt assembly module:** `Ahentic_Prompt_Assembler` owns system prompt text, history/tool compaction, and user-turn shaping (including slim AHENTIC_DEBUG recovery prompts). Think/Debug calls `for_llm()` for each LLM phase (or the assembler's tested helpers). Do not reimplement prompt/payload assembly at call sites.

**Plan module:** `Ahentic_Plan` owns the plan checklist. Primary interface: `sync_after_think()`, `ensure_after_think()`, `advance_after_tool()`, `complete_on_finish()`, `pause_for_user()`, `cancel_on_stop()`, `reopen_cancelled_steps()`. Think/Debug calls `sync_after_think()` / `ensure_after_think()` after a usable think (plan-missing retry is an internal Think/Debug LLM phase, trace `reason=plan_missing`). The Orchestrator (and Finish Gate for advance) must call the rest; do not reimplement plan merge/normalize/FSM at call sites. `ask_user` finishes must call `pause_for_user()` (not `complete_on_finish()`).
...
**Subagent module:** `Ahentic_Subagent` owns temporary cheap mode over **existing** abilities: deepen `forced_tools` (purpose `apply|batch|recipe` — successful queues may finish without a wrap-up think; Job Resume decides), preserve batch remainder across HITL/browser, bind placeholders from earlier step payloads before pause/execute, optional chain summary when idle, and **mini-job hop** (one slim think with main-packed `hop_brief` + normal ability catalog). Primary interface: `before_think()`, `after_main_think()`, `after_tools()`, `after_resume_tool()`, `on_think_failure()`, `on_tool_failure()`. The Orchestrator must call these — do not sequence hop phases (`try_begin_hop` / `mark_hop_running_tools` / `finish_hop_after_tools` / scattered `abort_hop`) at call sites. Recipe helpers for the Tool runner (`preserve_batch_remainder`, `bind_recipe_input`, `after_tool_success`) stay callable there. It must **not** invent follow-up abilities or domain recipes (no generate→upload→place hardcoding). Product should: [Agent runtime PRD — Subagent](../../pro__premium_only/docs/prd/agent-runtime.md#subagent-recipe--shipped).

**Think/debug module:** `Ahentic_Think_Debug` owns every non-tool LLM call for a step, AHENTIC_DEBUG recovery, and post-think disposition. Primary interface: `run_think()`, `apply_live_progress()`, `finalize_result_text()`, `should_finish_without_tools()`, `publish_thought_process()`, `queue_missing_ability()`, `normalize_tool_calls()`. Internal retries (attempt ≥2) must use a slim `for_llm( …, slim_debug_retry )` prompt - not a second full backpack. Slim recovery must not decide capability gaps (`missing_ability`); after a usable think that signals `missing_ability`, challenge the claim (rewrite to `use_tools` when the ability already appears in `tools_planned` with args, otherwise one full-catalog reconsider with `full_ability_catalog` so pack-gated tools are visible) before finishing or queuing a capability request. Never inject an empty `tools_planned` call for an available ability - that yields a useless HITL card. Queue capability requests only for concrete unknown ability slugs - not vague / `ahentic/new-ability` placeholders. After a usable think, fold plan-missing retry into the same module (`bump_step=false`; hop thinks skip plan enforcement). The Orchestrator must call `run_think()` (and the disposition helpers) - do not reimplement debug retry, missing-ability policy, plan-missing LLM retry, or a second LLM phase at call sites. Forced-tools steps are not LLM thinks.
